    ...
    try:
        ...
        if st.button("🔍 Run Sentiment Analysis"):
            if selected_col:
                ...
                st.warning("Please select a text column from the dropdown to run sentiment analysis. ⚠️")
...
if st.session_state["sentiment_df"] is not None and not st.session_state["sentiment_df"].empty:
    ...
    if not clean_comments_for_lda:
        st.warning("No valid cleaned text data available for Topic Modeling after initial filtering. Please ensure your data contains meaningful text. ℹ️")
    ...
    # Filter out any lists that became empty after simple_preprocess
    final_processed_texts = [text for text in processed_texts if text]

    if not final_processed_texts:
        st.warning("No valid processed text data for Topic Modeling after detailed preprocessing. This might happen if all comments consist only of stopwords or short words. ℹ️")
    ...
    # Update session state for LDA components
        ...
        if submit_labels:
            ...
            for idx, row in df_with_topics.iterrows():
                ...
                try:
                    ...
                    st.warning(f"Could not assign topic to document at index {idx} due to vocabulary mismatch. Assigning 'Unassigned'. ⚠️")
                    full_topic_assignments[idx] = "Unassigned"
                    continue
            ...
            # Filter out 'Unassigned' topics for visualization purposes
            ...
            if not df_for_viz.empty:
                ...
                if df_sim_ready.empty:
                    st.warning("Cannot generate topic relationship graph: No data for sentiment similarity calculation after filtering. ⚠️")
                elif len(df_sim_ready) < 2:
                    st.info("Not enough topics (at least 2 required) to draw a relationship graph. ℹ️")
                else:
                    ...
                    # Add edges for unique pairs, applying the 0.5 similarity threshold
                    for i in range(len(topic_polarity_matrix)):
                        for j in range(i + 1, len(topic_polarity_matrix[0])): 
                            if topic_polarity_matrix[i][j] > 0.5: # Only add edges above the threshold
                    ...
                    if edge_weights: # Only proceed if there are edges
                        ...
                        widths = [] # No edges, so no widths

                    # Draw the graph (this block is *not* wrapped by 'if not G.edges:')
                    ...
                    # --- End of NetworkX Graph Code ---
                                
                # --- Interactive LDA Visualization (pyLDAvis) ---
                st.subheader("📈 Interactive LDA Visualization (pyLDAvis)")
                st.info("This interactive visualization helps explore topics by showing their relationships and the most relevant terms. Move the mouse over topics and words for details. 🖱️")
                ...
                if st.session_state["lda_model"] and st.session_state["corpus"] and st.session_state["id2word"]:
                    ...
                    st.warning("LDA model or its components are not ready for interactive visualization. Please run LDA training first. ⚠️")
            else:
